"use strict";
(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
  [2694],
  {
    20794: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("ArrowRightSquare", [
        [
          "rect",
          { width: "18", height: "18", x: "3", y: "3", rx: "2", key: "afitv7" },
        ],
        ["path", { d: "M8 12h8", key: "1wcyev" }],
        ["path", { d: "m12 16 4-4-4-4", key: "1i9zcv" }],
      ]);
    },
    26490: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("Clock", [
        ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
        ["polyline", { points: "12 6 12 12 16 14", key: "68esgv" }],
      ]);
    },
    3526: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("ListMusic", [
        ["path", { d: "M21 15V6", key: "h1cx4g" }],
        [
          "path",
          { d: "M18.5 18a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z", key: "8saifv" },
        ],
        ["path", { d: "M12 12H3", key: "18klou" }],
        ["path", { d: "M16 6H3", key: "1wxfjs" }],
        ["path", { d: "M12 18H3", key: "11ftsu" }],
      ]);
    },
    32276: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("MessageSquareMore", [
        [
          "path",
          {
            d: "M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z",
            key: "1lielz",
          },
        ],
        ["path", { d: "M8 10h.01", key: "19clt8" }],
        ["path", { d: "M12 10h.01", key: "1nrarc" }],
        ["path", { d: "M16 10h.01", key: "1m94wz" }],
      ]);
    },
    97373: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("MoreHorizontal", [
        ["circle", { cx: "12", cy: "12", r: "1", key: "41hilf" }],
        ["circle", { cx: "19", cy: "12", r: "1", key: "1wjl8i" }],
        ["circle", { cx: "5", cy: "12", r: "1", key: "1pcz8c" }],
      ]);
    },
    25310: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("PlusCircle", [
        ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
        ["path", { d: "M8 12h8", key: "1wcyev" }],
        ["path", { d: "M12 8v8", key: "napkw2" }],
      ]);
    },
    41145: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("Split", [
        ["path", { d: "M16 3h5v5", key: "1806ms" }],
        ["path", { d: "M8 3H3v5", key: "15dfkv" }],
        ["path", { d: "M12 22v-8.3a4 4 0 0 0-1.172-2.872L3 3", key: "1qrqzj" }],
        ["path", { d: "m15 9 6-6", key: "ko1vev" }],
      ]);
    },
    37947: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("ThumbsDown", [
        ["path", { d: "M17 14V2", key: "8ymqnk" }],
        [
          "path",
          {
            d: "M9 18.12 10 14H4.17a2 2 0 0 1-1.92-2.56l2.33-8A2 2 0 0 1 6.5 2H20a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-2.76a2 2 0 0 0-1.79 1.11L12 22h0a3.13 3.13 0 0 1-3-3.88Z",
            key: "s6e0r",
          },
        ],
      ]);
    },
    75707: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("ThumbsUp", [
        ["path", { d: "M7 10v12", key: "1qc93n" }],
        [
          "path",
          {
            d: "M15 5.88 14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-1.11L12 2h0a3.13 3.13 0 0 1 3 3.88Z",
            key: "y3tblf",
          },
        ],
      ]);
    },
    50489: function (e, t, n) {
      n.d(t, {
        Z: function () {
          return r;
        },
      }); /**
       * @license lucide-react v0.303.0 - ISC
       *
       * This source code is licensed under the ISC license.
       * See the LICENSE file in the root directory of this source tree.
       */
      let r = (0, n(87461).Z)("Trash2", [
        ["path", { d: "M3 6h18", key: "d0wm0j" }],
        ["path", { d: "M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6", key: "4alrt4" }],
        ["path", { d: "M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2", key: "v07s0e" }],
        ["line", { x1: "10", x2: "10", y1: "11", y2: "17", key: "1uufr5" }],
        ["line", { x1: "14", x2: "14", y1: "11", y2: "17", key: "xtxkd" }],
      ]);
    },
    14522: function (e, t, n) {
      n.d(t, {
        QS: function () {
          return l;
        },
      });
      var r = n(2265);
      let o = {
          delta: 10,
          preventScrollOnSwipe: !1,
          rotationAngle: 0,
          trackMouse: !1,
          trackTouch: !0,
          swipeDuration: 1 / 0,
          touchEventOptions: { passive: !0 },
        },
        a = { first: !0, initial: [0, 0], start: 0, swiping: !1, xy: [0, 0] },
        i = "mousemove",
        c = "mouseup";
      function u(e, t) {
        if (0 === t) return e;
        let n = (Math.PI / 180) * t;
        return [
          e[0] * Math.cos(n) + e[1] * Math.sin(n),
          e[1] * Math.cos(n) - e[0] * Math.sin(n),
        ];
      }
      function l(e) {
        var t, n, l;
        let s;
        let { trackMouse: f } = e,
          p = r.useRef(Object.assign({}, a)),
          d = r.useRef(Object.assign({}, o)),
          h = r.useRef(Object.assign({}, d.current));
        for (s in ((h.current = Object.assign({}, d.current)),
        (d.current = Object.assign(Object.assign({}, o), e)),
        o))
          void 0 === d.current[s] && (d.current[s] = o[s]);
        let [g, v] = r.useMemo(
          () =>
            (function (e, t) {
              let n = (t) => {
                  let n = "touches" in t;
                  (n && t.touches.length > 1) ||
                    e((e, o) => {
                      o.trackMouse &&
                        !n &&
                        (document.addEventListener(i, r),
                        document.addEventListener(c, f));
                      let { clientX: l, clientY: s } = n ? t.touches[0] : t,
                        p = u([l, s], o.rotationAngle);
                      return (
                        o.onTouchStartOrOnMouseDown &&
                          o.onTouchStartOrOnMouseDown({ event: t }),
                        Object.assign(Object.assign(Object.assign({}, e), a), {
                          initial: p.slice(),
                          xy: p,
                          start: t.timeStamp || 0,
                        })
                      );
                    });
                },
                r = (t) => {
                  e((e, n) => {
                    let r = "touches" in t;
                    if (r && t.touches.length > 1) return e;
                    if (t.timeStamp - e.start > n.swipeDuration)
                      return e.swiping
                        ? Object.assign(Object.assign({}, e), { swiping: !1 })
                        : e;
                    let { clientX: a, clientY: i } = r ? t.touches[0] : t,
                      [c, l] = u([a, i], n.rotationAngle),
                      s = c - e.xy[0],
                      f = l - e.xy[1],
                      p = Math.abs(s),
                      d = Math.abs(f),
                      h = (t.timeStamp || 0) - e.start,
                      g =
                        p > d
                          ? s > 0
                            ? "Right"
                            : "Left"
                          : f > 0
                            ? "Down"
                            : "Up",
                      v =
                        "number" == typeof n.delta
                          ? n.delta
                          : n.delta[g.toLowerCase()] || o.delta;
                    if (p < v && d < v && !e.swiping) return e;
                    let m = {
                      absX: p,
                      absY: d,
                      deltaX: s,
                      deltaY: f,
                      dir: g,
                      event: t,
                      first: e.first,
                      initial: e.initial,
                      velocity: Math.sqrt(p * p + d * d) / (h || 1),
                      vxvy: [s / (h || 1), f / (h || 1)],
                    };
                    m.first && n.onSwipeStart && n.onSwipeStart(m),
                      n.onSwiping && n.onSwiping(m);
                    let x = !1;
                    return (
                      (n.onSwiping || n.onSwiped || n[`onSwiped${g}`]) &&
                        (x = !0),
                      x &&
                        n.preventScrollOnSwipe &&
                        n.trackTouch &&
                        t.cancelable &&
                        t.preventDefault(),
                      Object.assign(Object.assign({}, e), {
                        first: !1,
                        eventData: m,
                        swiping: !0,
                      })
                    );
                  });
                },
                l = (t) => {
                  e((e, n) => {
                    let r;
                    if (e.swiping && e.eventData) {
                      if (t.timeStamp - e.start < n.swipeDuration) {
                        (r = Object.assign(Object.assign({}, e.eventData), {
                          event: t,
                        })),
                          n.onSwiped && n.onSwiped(r);
                        let o = n[`onSwiped${r.dir}`];
                        o && o(r);
                      }
                    } else n.onTap && n.onTap({ event: t });
                    return (
                      n.onTouchEndOrOnMouseUp &&
                        n.onTouchEndOrOnMouseUp({ event: t }),
                      Object.assign(Object.assign(Object.assign({}, e), a), {
                        eventData: r,
                      })
                    );
                  });
                },
                s = () => {
                  document.removeEventListener(i, r),
                    document.removeEventListener(c, f);
                },
                f = (e) => {
                  s(), l(e);
                },
                p = (e, t) => {
                  let a = () => {};
                  if (e && e.addEventListener) {
                    let i = Object.assign(
                        Object.assign({}, o.touchEventOptions),
                        t.touchEventOptions
                      ),
                      c = [
                        ["touchstart", n, i],
                        [
                          "touchmove",
                          r,
                          Object.assign(
                            Object.assign({}, i),
                            t.preventScrollOnSwipe ? { passive: !1 } : {}
                          ),
                        ],
                        ["touchend", l, i],
                      ];
                    c.forEach(([t, n, r]) => e.addEventListener(t, n, r)),
                      (a = () =>
                        c.forEach(([t, n]) => e.removeEventListener(t, n)));
                  }
                  return a;
                },
                d = {
                  ref: (t) => {
                    null !== t &&
                      e((e, n) => {
                        if (e.el === t) return e;
                        let r = {};
                        return (
                          e.el &&
                            e.el !== t &&
                            e.cleanUpTouch &&
                            (e.cleanUpTouch(), (r.cleanUpTouch = void 0)),
                          n.trackTouch && t && (r.cleanUpTouch = p(t, n)),
                          Object.assign(
                            Object.assign(Object.assign({}, e), { el: t }),
                            r
                          )
                        );
                      });
                  },
                };
              return t.trackMouse && (d.onMouseDown = n), [d, p];
            })((e) => (p.current = e(p.current, d.current)), { trackMouse: f }),
          [f]
        );
        return (
          (p.current =
            ((t = p.current),
            (n = d.current),
            (l = h.current),
            n.trackTouch && t.el
              ? t.cleanUpTouch
                ? n.preventScrollOnSwipe !== l.preventScrollOnSwipe ||
                  n.touchEventOptions.passive !== l.touchEventOptions.passive
                  ? (t.cleanUpTouch(),
                    Object.assign(Object.assign({}, t), {
                      cleanUpTouch: v(t.el, n),
                    }))
                  : t
                : Object.assign(Object.assign({}, t), {
                    cleanUpTouch: v(t.el, n),
                  })
              : (t.cleanUpTouch && t.cleanUpTouch(),
                Object.assign(Object.assign({}, t), {
                  cleanUpTouch: void 0,
                })))),
          g
        );
      }
    },
    65687: function (e, t, n) {
      n.d(t, {
        $j: function () {
          return j;
        },
        Dx: function () {
          return D;
        },
        VY: function () {
          return C;
        },
        aU: function () {
          return R;
        },
        aV: function () {
          return E;
        },
        dk: function () {
          return P;
        },
        fC: function () {
          return k;
        },
        h_: function () {
          return Z;
        },
        xz: function () {
          return _;
        },
      });
      var r = n(14749),
        o = n(2265),
        a = n(84104),
        i = n(61266),
        c = n(72936),
        u = n(44991),
        l = n(59143);
      let [s, f] = (0, a.b)("AlertDialog", [c.p8]),
        p = (0, c.p8)(),
        d = (0, o.forwardRef)((e, t) => {
          let { __scopeAlertDialog: n, ...a } = e,
            i = p(n);
          return (0, o.createElement)(c.xz, (0, r.Z)({}, i, a, { ref: t }));
        }),
        h = (0, o.forwardRef)((e, t) => {
          let { __scopeAlertDialog: n, ...a } = e,
            i = p(n);
          return (0, o.createElement)(c.aV, (0, r.Z)({}, i, a, { ref: t }));
        }),
        g = "AlertDialogContent",
        [v, m] = s(g),
        x = (0, o.forwardRef)((e, t) => {
          let { __scopeAlertDialog: n, children: a, ...s } = e,
            f = p(n),
            d = (0, o.useRef)(null),
            h = (0, i.e)(t, d),
            m = (0, o.useRef)(null);
          return (0, o.createElement)(
            c.jm,
            { contentName: g, titleName: w, docsSlug: "alert-dialog" },
            (0, o.createElement)(
              v,
              { scope: n, cancelRef: m },
              (0, o.createElement)(
                c.VY,
                (0, r.Z)({ role: "alertdialog" }, f, s, {
                  ref: h,
                  onOpenAutoFocus: (0, u.M)(s.onOpenAutoFocus, (e) => {
                    var t;
                    e.preventDefault(),
                      null === (t = m.current) ||
                        void 0 === t ||
                        t.focus({ preventScroll: !0 });
                  }),
                  onPointerDownOutside: (e) => e.preventDefault(),
                  onInteractOutside: (e) => e.preventDefault(),
                }),
                (0, o.createElement)(l.A4, null, a),
                !1
              )
            )
          );
        }),
        w = "AlertDialogTitle",
        M = (0, o.forwardRef)((e, t) => {
          let { __scopeAlertDialog: n, ...a } = e,
            i = p(n);
          return (0, o.createElement)(c.Dx, (0, r.Z)({}, i, a, { ref: t }));
        }),
        O = (0, o.forwardRef)((e, t) => {
          let { __scopeAlertDialog: n, ...a } = e,
            i = p(n);
          return (0, o.createElement)(c.dk, (0, r.Z)({}, i, a, { ref: t }));
        }),
        y = (0, o.forwardRef)((e, t) => {
          let { __scopeAlertDialog: n, ...a } = e,
            i = p(n);
          return (0, o.createElement)(c.x8, (0, r.Z)({}, i, a, { ref: t }));
        }),
        b = (0, o.forwardRef)((e, t) => {
          let { __scopeAlertDialog: n, ...a } = e,
            { cancelRef: u } = m("AlertDialogCancel", n),
            l = p(n),
            s = (0, i.e)(t, u);
          return (0, o.createElement)(c.x8, (0, r.Z)({}, l, a, { ref: s }));
        }),
        k = (e) => {
          let { __scopeAlertDialog: t, ...n } = e,
            a = p(t);
          return (0, o.createElement)(c.fC, (0, r.Z)({}, a, n, { modal: !0 }));
        },
        _ = d,
        Z = (e) => {
          let { __scopeAlertDialog: t, ...n } = e,
            a = p(t);
          return (0, o.createElement)(c.h_, (0, r.Z)({}, a, n));
        },
        E = h,
        C = x,
        R = y,
        j = b,
        D = M,
        P = O;
    },
    16821: function (e, t, n) {
      n.d(t, {
        Ee: function () {
          return V;
        },
        Rk: function () {
          return z;
        },
        Tr: function () {
          return F;
        },
        Uv: function () {
          return P;
        },
        VY: function () {
          return S;
        },
        Z0: function () {
          return H;
        },
        ZA: function () {
          return T;
        },
        __: function () {
          return A;
        },
        ck: function () {
          return U;
        },
        fC: function () {
          return j;
        },
        fF: function () {
          return q;
        },
        oC: function () {
          return L;
        },
        tu: function () {
          return W;
        },
        wU: function () {
          return N;
        },
        xz: function () {
          return D;
        },
      });
      var r = n(14749),
        o = n(2265),
        a = n(44991),
        i = n(84104),
        c = n(29586),
        u = n(33660),
        l = n(39830),
        s = n(9310);
      let f = "ContextMenu",
        [p, d] = (0, i.b)(f, [u.Wf]),
        h = (0, u.Wf)(),
        [g, v] = p(f),
        m = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, disabled: i = !1, ...l } = e,
            s = v("ContextMenuTrigger", n),
            f = h(n),
            p = (0, o.useRef)({ x: 0, y: 0 }),
            d = (0, o.useRef)({
              getBoundingClientRect: () =>
                DOMRect.fromRect({ width: 0, height: 0, ...p.current }),
            }),
            g = (0, o.useRef)(0),
            m = (0, o.useCallback)(() => window.clearTimeout(g.current), []),
            x = (e) => {
              (p.current = { x: e.clientX, y: e.clientY }), s.onOpenChange(!0);
            };
          return (
            (0, o.useEffect)(() => m, [m]),
            (0, o.useEffect)(() => void (i && m()), [i, m]),
            (0, o.createElement)(
              o.Fragment,
              null,
              (0, o.createElement)(u.ee, (0, r.Z)({}, f, { virtualRef: d })),
              (0, o.createElement)(
                c.WV.span,
                (0, r.Z)(
                  {
                    "data-state": s.open ? "open" : "closed",
                    "data-disabled": i ? "" : void 0,
                  },
                  l,
                  {
                    ref: t,
                    style: { WebkitTouchCallout: "none", ...e.style },
                    onContextMenu: i
                      ? e.onContextMenu
                      : (0, a.M)(e.onContextMenu, (e) => {
                          m(), x(e), e.preventDefault();
                        }),
                    onPointerDown: i
                      ? e.onPointerDown
                      : (0, a.M)(
                          e.onPointerDown,
                          R((e) => {
                            m(),
                              (g.current = window.setTimeout(() => x(e), 700));
                          })
                        ),
                    onPointerMove: i
                      ? e.onPointerMove
                      : (0, a.M)(e.onPointerMove, R(m)),
                    onPointerCancel: i
                      ? e.onPointerCancel
                      : (0, a.M)(e.onPointerCancel, R(m)),
                    onPointerUp: i
                      ? e.onPointerUp
                      : (0, a.M)(e.onPointerUp, R(m)),
                  }
                )
              )
            )
          );
        }),
        x = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = v("ContextMenuContent", n),
            c = h(n),
            l = (0, o.useRef)(!1);
          return (0, o.createElement)(
            u.VY,
            (0, r.Z)({}, c, a, {
              ref: t,
              side: "right",
              sideOffset: 2,
              align: "start",
              onCloseAutoFocus: (t) => {
                var n;
                null === (n = e.onCloseAutoFocus) ||
                  void 0 === n ||
                  n.call(e, t),
                  !t.defaultPrevented && l.current && t.preventDefault(),
                  (l.current = !1);
              },
              onInteractOutside: (t) => {
                var n;
                null === (n = e.onInteractOutside) ||
                  void 0 === n ||
                  n.call(e, t),
                  t.defaultPrevented || i.modal || (l.current = !0);
              },
              style: {
                ...e.style,
                "--radix-context-menu-content-transform-origin":
                  "var(--radix-popper-transform-origin)",
                "--radix-context-menu-content-available-width":
                  "var(--radix-popper-available-width)",
                "--radix-context-menu-content-available-height":
                  "var(--radix-popper-available-height)",
                "--radix-context-menu-trigger-width":
                  "var(--radix-popper-anchor-width)",
                "--radix-context-menu-trigger-height":
                  "var(--radix-popper-anchor-height)",
              },
            })
          );
        }),
        w = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.ZA, (0, r.Z)({}, i, a, { ref: t }));
        }),
        M = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.__, (0, r.Z)({}, i, a, { ref: t }));
        }),
        O = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.ck, (0, r.Z)({}, i, a, { ref: t }));
        }),
        y = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.oC, (0, r.Z)({}, i, a, { ref: t }));
        }),
        b = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.Ee, (0, r.Z)({}, i, a, { ref: t }));
        }),
        k = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.Rk, (0, r.Z)({}, i, a, { ref: t }));
        }),
        _ = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.wU, (0, r.Z)({}, i, a, { ref: t }));
        }),
        Z = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.Z0, (0, r.Z)({}, i, a, { ref: t }));
        }),
        E = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(u.fF, (0, r.Z)({}, i, a, { ref: t }));
        }),
        C = (0, o.forwardRef)((e, t) => {
          let { __scopeContextMenu: n, ...a } = e,
            i = h(n);
          return (0, o.createElement)(
            u.tu,
            (0, r.Z)({}, i, a, {
              ref: t,
              style: {
                ...e.style,
                "--radix-context-menu-content-transform-origin":
                  "var(--radix-popper-transform-origin)",
                "--radix-context-menu-content-available-width":
                  "var(--radix-popper-available-width)",
                "--radix-context-menu-content-available-height":
                  "var(--radix-popper-available-height)",
                "--radix-context-menu-trigger-width":
                  "var(--radix-popper-anchor-width)",
                "--radix-context-menu-trigger-height":
                  "var(--radix-popper-anchor-height)",
              },
            })
          );
        });
      function R(e) {
        return (t) => ("mouse" !== t.pointerType ? e(t) : void 0);
      }
      let j = (e) => {
          let {
              __scopeContextMenu: t,
              children: n,
              onOpenChange: a,
              dir: i,
              modal: c = !0,
            } = e,
            [s, f] = (0, o.useState)(!1),
            p = h(t),
            d = (0, l.W)(a),
            v = (0, o.useCallback)(
              (e) => {
                f(e), d(e);
              },
              [d]
            );
          return (0, o.createElement)(
            g,
            { scope: t, open: s, onOpenChange: v, modal: c },
            (0, o.createElement)(
              u.fC,
              (0, r.Z)({}, p, { dir: i, open: s, onOpenChange: v, modal: c }),
              n
            )
          );
        },
        D = m,
        P = (e) => {
          let { __scopeContextMenu: t, ...n } = e,
            a = h(t);
          return (0, o.createElement)(u.h_, (0, r.Z)({}, a, n));
        },
        S = x,
        T = w,
        A = M,
        U = O,
        L = y,
        V = b,
        z = k,
        N = _,
        H = Z,
        F = (e) => {
          let {
              __scopeContextMenu: t,
              children: n,
              onOpenChange: a,
              open: i,
              defaultOpen: c,
            } = e,
            l = h(t),
            [f, p] = (0, s.T)({ prop: i, defaultProp: c, onChange: a });
          return (0, o.createElement)(
            u.Tr,
            (0, r.Z)({}, l, { open: f, onOpenChange: p }),
            n
          );
        },
        q = E,
        W = C;
    },
    34050: function (e, t, n) {
      n.d(t, {
        N: function () {
          return c;
        },
      });
      var r = n(86968),
        o = n(65612),
        a = class extends r.z {
          constructor(e, t) {
            super(e, t);
          }
          bindMethods() {
            super.bindMethods(),
              (this.fetchNextPage = this.fetchNextPage.bind(this)),
              (this.fetchPreviousPage = this.fetchPreviousPage.bind(this));
          }
          setOptions(e, t) {
            super.setOptions({ ...e, behavior: (0, o.Gm)() }, t);
          }
          getOptimisticResult(e) {
            return (e.behavior = (0, o.Gm)()), super.getOptimisticResult(e);
          }
          fetchNextPage(e) {
            return this.fetch({
              ...e,
              meta: { fetchMore: { direction: "forward" } },
            });
          }
          fetchPreviousPage(e) {
            return this.fetch({
              ...e,
              meta: { fetchMore: { direction: "backward" } },
            });
          }
          createResult(e, t) {
            let { state: n } = e,
              r = super.createResult(e, t),
              { isFetching: a, isRefetching: i } = r,
              c = a && n.fetchMeta?.fetchMore?.direction === "forward",
              u = a && n.fetchMeta?.fetchMore?.direction === "backward";
            return {
              ...r,
              fetchNextPage: this.fetchNextPage,
              fetchPreviousPage: this.fetchPreviousPage,
              hasNextPage: (0, o.Qy)(t, n.data),
              hasPreviousPage: (0, o.ZF)(t, n.data),
              isFetchingNextPage: c,
              isFetchingPreviousPage: u,
              isRefetching: i && !c && !u,
            };
          }
        },
        i = n(22438);
      function c(e, t) {
        return (0, i.r)(e, a, t);
      }
    },
  },
]);
