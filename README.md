...
Analyzing the evolution of Udio's codebase through time with curated archives and scripts.

Or, to put it more poetically, in the eloquent words of ChatGPT:

> Udio Source Watch is a meticulously curated repository that serves as a treasure trove for those interested in observing the evolution of Udio's webpack chunks. It gracefully preserves the historical webpack chunks in their original splendor, while also offering a breath of fresh air with unpacked and beautifully formatted versions of the chunk files. This thoughtful touch empowers you to effortlessly analyze the nuances between different builds.
>
...
>
> Udio Source Watch stands as a beacon of transparency and a portal to discovery.
...
If you're looking for a concise summary of the changes and updates in Udio's codebase over time, the [`CHANGELOG.md`](./CHANGELOG.md) is probably what you need. It's a comprehensive record of changes made in each build version and serves as a quick reference.
...
- Original URL: https://web.archive.org/web/*/https://www.udio.com/_next/*
...
- [`buildmanifest-to-json.js`](./scripts/buildmanifest-to-json.js): A Node.js script that converts a build manifest file to JSON. When piped with the second `jq` command, it extracts and prefixes static asset URLs with `https://www.udio.com/_next/`, ready for fetching.
...
```shell
git clone https://github.com/0xdevalias/udio-source-watch.git
cd udio-source-watch
```
...
- [**React Server Components, Next.js v13+, and Webpack: Notes on Streaming Wire Format (`__next_f`, etc)**)](https://gist.github.com/0xdevalias/ac465fb2f7e6fded183c2a4273d21e61#react-server-components-nextjs-v13-and-webpack-notes-on-streaming-wire-format-__next_f-etc): Some notes on React Server Components (specifically the streaming wire format), Next.js v13+ (specifically `__next_f`), and Webpack
- [**Decrypting GrowthBook feature flags**](https://gist.github.com/0xdevalias/ca22836755c2e1024b1f9c36504d56a3#decrypting-growthbook-feature-flags): Some notes on decrypting GrowthBook feature flags
- [**Fingerprinting Minified JavaScript Libraries / AST Fingerprinting / Source Code Similarity / Etc**](https://gist.github.com/0xdevalias/31c6574891db3e36f15069b859065267#fingerprinting-minified-javascript-libraries--ast-fingerprinting--source-code-similarity--etc): Some notes and tools on fingerprinting minified JavaScript libraries, AST fingerprinting, source code similarity, etc
...
[![Star History Chart](https://api.star-history.com/svg?repos=0xdevalias/udio-source-watch&type=Date)](https://star-history.com/#0xdevalias/udio-source-watch&Date)
